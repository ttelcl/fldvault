...
let usage focus =
  ...
  if showDetail "push" then
    ...
    cp ""
  if showSection "delta" then
    cp "\fogitvault \fydelta new\f0 \fg...\f0"
    cp "\fogitvault \fydelta edit\f0 \fg...\f0"
    cp "\fogitvault \fydelta drop\f0 \fg-r \fcrecipe\f0"
    cp "\fogitvault \fydelta default\f0 \fg-r \fcrecipe\f0"
    cp "\fogitvault \fydelta send\f0 [\fg-r \fcrecipe\f0]"
    cp "  Delta recipe management and use. See '\fogitvault \fydelta \fg-h\f0' for more details"
  if showDetail "delta" then
    cp "(\frdocumentation WIP\f0)"
    cp ""
